...
class Agent:
    ...
    def __init__(self, env: game.Game):
        ...
        self.model.add(keras.layers.Dense(24, input_shape=[env.observations], activation='relu', kernel_initializer=init))
    ...
    def train(self, replay_memory, target_model, done):
        ...
        current_states = np.array(
            [self.encode_observation(transition[0], self.env.observations) for transition in mini_batch], dtype=int)

        current_qs_list = self.model.predict(current_states)
        new_current_states = np.array(
            [self.encode_observation(transition[3], self.env.observations) for transition in mini_batch], dtype=int)
